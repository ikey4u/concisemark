    ...
    pub fn new(lines: &[&str], indent: usize) -> Result<Self> {
        ...
        while pos < chars.len() {
            if let Some(mark) = super::Mark::new(&chars[pos..]) {
                let t = chars[pos..].iter().map(|c| c.to_string()).collect::<String>();
                let t = &t[..mark.size].to_string();
                para += &t;
                pos += t.chars().count();
            } else if let Some(pair) = Pair::new(&chars[pos..], '`') {
                para += &format!("{}{}{}", pair.boundaries, pair.content, pair.boundaries);
                pos += pair.number_of_char;
            } else {
                if chars[pos] == '\n' {
                    let nextline: String = chars[pos + 1..].iter()
                        .take_while(|&&x| x != '\n')
                        .map(|c| c.to_string())
                        .collect();
